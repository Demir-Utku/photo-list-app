...
export default class View extends Component {
    ...
    axios
      .get(
        "https://api.unsplash.com/photos/" +
          id +
          "?client_id=oGAB55r3jWyuk5FXcJEV2kbB2DBvVmPfHowiFHHoTn4"
      )
      .then((response) => {
        let location = false;
  ...
  getImage = () => {
    return (
      <>
        <a
          href={`#${this.props.view.id}`}
          onLoad={() => this.loadDetail(this.props.view.id)}
        >
          <img
            src={this.props.view.url}
            style={{
              height: `${
                20.5 * (this.props.view.height / this.props.view.width)
              }vw`,
            }}
            key={this.props.view.url}
          />
        </a>
        {this.state.searched && (
          <Detail
  ...
  render() {
    return <div className="image-wrapper">{this.getImage()}</div>;
  }
}
